...
class MyTensors():
    def __init__(self, config: MyConfig):
        ...
        with tf.device("/gpu:0"):
            ...
            self.labels = tf.placeholder(dtype=tf.int32, shape=[None], name="y")  # [-1]
            self.lr = tf.placeholder(dtype=tf.float32, shape=None, name="lr")  #
            self.inputs = [self.xs, self.labels, self.lr]

            x = tf.reshape(self.xs, [-1, 28, 28, 1])
            self.vec = self.encode(x, config.vec_size)
            y = self.decode(self.vec, self.labels)  # [-1, 28, 28, 1]
            # 计算vec的平均值
    ...
    def decode(self, vec, labels):
        ...
        """""
        # todo 添加 labels
        labels = tf.one_hot(labels, 10)  # [-1, 10]
        y = tf.layers.dense(vec, units=7 * 7 * 64, name="de_dense")  # [-1, 7 * 7 * 64]
        l = tf.layers.dense(labels, 7 * 7 * 64)
        ...
        l = tf.layers.dense(labels, 14 * 14 * 32)
        l = tf.reshape(l, [-1, 14, 14, 32])
        y += l
        y = tf.layers.conv2d_transpose(y, 16, 3, strides=2, padding="same", activation=tf.nn.relu,
                                       name="de_conv2_1")  # [-1, 28, 28, 16]
        l = tf.layers.dense(labels, 28 * 28 * 16)
        l = tf.reshape(l, [-1, 28, 28, 16])
        y += l
...
def predict(app, sample, path, col):
    ...
    labels = [im % 10 for im in range(sample)]
    vec = np.random.normal(mean, std, [sample, len(std)])
    images = app.session.run(app.ts.y, {app.ts.vec: vec, app.ts.labels: labels})  # [-1, 28, 28]
    # 将所有图片做成一张图片，每行20张小图
    images = np.reshape(images, [-1, col, 28, 28])  # [-1, 20, 28, 28]
    images = np.transpose(images, [0, 2, 1, 3])  # [-1, 28, 20, 28]
    images = np.reshape(images, [-1, col * 28])  # [-1, 20 * 28]
